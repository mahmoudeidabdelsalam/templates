<?php
class BannerLeft extends WP_Widget {

    function __construct() {
        $widget_options = array();
        parent::__construct(false, __('#Premast Banner Left', 'premast') , $widget_options);
    }

    function form($instance) {
        $defaults = array(
            'title' => __('Banner Left', 'premast'),
        );
        $instance = wp_parse_args((array) $instance, $defaults);
      ?>

        <p>
          <img src='<?= get_theme_file_uri().'/resources/assets/images/widgets/banner-left.png'; ?>' style="width:100%;" />
        </p>
        <p>
