...
Description:  A Like Post for WordPress.
Version:      1.0.0
Author:       Mahmoud Eid
Author URI:   http://code95.com/
License:
Copyright (C) 2015 Code95
*/
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_process_comments_like', 'process_comments_like' );
add_action( 'wp_ajax_process_comments_like', 'process_comments_like' );
function process_comments_like() {
	// Security
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'YourThemeTextDomain' ) );
	...
	if ( $post_id != '' ) {
		...
		if ( !already_liked( $post_id, $is_comment ) ) { // Like the post
			...
			$like_count = ++$count;
			$response['status'] = "liked";
			$response['icon'] = get_comment_liked_icon();
		} else { // Unlike the post
			if ( is_user_logged_in() ) { // user is logged in
				$user_id = get_current_user_id();
				$post_users = post_user_likes( $user_id, $post_id, $is_comment );
			...
			$like_count = ( $count > 0 ) ? --$count : 0; // Prevent negative number
			$response['status'] = "unliked";
			$response['icon'] = get_comment_unliked_icon();
		}
		if ( $is_comment == 1 ) {
			update_comment_meta( $post_id, "_comment_like_count", $like_count );
			update_comment_meta( $post_id, "_comment_like_modified", date( 'Y-m-d H:i:s' ) );
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	// Loader
	$loader = '<span id="sl-loader" style="display:none;"><i class="fa fa-spinner fa-spin" aria-hidden="true"></i></span>';
 ...
 * @since    0.5
 */
function get_comment_liked_icon() {
 ...
 * @since    0.5
 */
function get_comment_unliked_icon() {
 ...
 * @since    0.5
 */
function get_comment_likes_button( $post_id, $is_comment = NULL ) {
	$is_comment = ( NULL == $is_comment ) ? 0 : 1;
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Security
	if ( $is_comment == 1 ) {
		...
		$post_id_class = esc_attr( ' sl-comment-' . $post_id );
		$comment_class = esc_attr( '' );
		$like_count = get_post_meta( $post_id, "_post_like_count", true );
		$like_count = ( isset( $like_count ) && is_numeric( $like_count ) ) ? $like_count : 0;
	}
	$count = get_like_count( $like_count );
	$icon_empty = get_comment_unliked_icon();
	$icon_full = get_comment_liked_icon();
	// Loader
	...
	$output = '<span class="like-wrapper"><a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="sl-comment' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . $count . '</a>' . $loader . '</span>';
	return $output;
} // get_simple_likes_button()
...
function get_like_count( $like_count ) {
	$like_text = '<span class="sl-text">' .  __( 'Like', 'YourThemeTextDomain' ) . '</span>';
	if ( is_numeric( $like_count ) && $like_count > 0 ) {
		$number = sl_format_count( $like_count ) . $like_text;
	} else {
		$number = $like_text;
	}
